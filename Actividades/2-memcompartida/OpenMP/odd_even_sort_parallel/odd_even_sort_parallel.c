...
void serial_odd_even_sort(size_t n, double arr[n], int number_threads)
{
	...
	for ( size_t phase = 0; phase < n; ++phase )
	{
		if ( phase % 2 == 0 )
		{
			#pragma omp parallel for num_threads(number_threads) default(none) shared(n, arr) 
...
int main(int argc, char* argv[])
{
	...
	serial_odd_even_sort(n, arr, number_threads);
	for(size_t index = 0; index < n; ++index)
	{
		printf("%lf ", arr[index]);
	}
